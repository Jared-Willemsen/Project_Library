...
class ForgotPasswordView(ctk.CTkFrame):
    def __init__(self, master):
        ...
        # populate frame
        self.logo_label = ctk.CTkLabel(self, text='  Alexandria Library', text_color='gray25',
                                       font=ctk.CTkFont(size=35, weight='bold'), image=self.logo_image,
                                       compound='left')
        self.logo_label.grid(row=0, column=0, padx=20, pady=25, columnspan=2)

        self.header = ctk.CTkLabel(self, text='  Reset password', text_color='gray25',
                                   font=ctk.CTkFont(size=25, weight='bold'))
        self.header.grid(row=1, column=0, padx=20, pady=25, columnspan=2)

        self.back_button = ctk.CTkButton(self, text='', image=self.arrow_back_image, width=40, height=30)
        self.back_button.grid(row=2, column=0, sticky='w', padx=20, pady=(0, 20))
