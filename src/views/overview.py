...
class OverviewView(ctk.CTkFrame):
    ...
    def __init__(self, master) -> None:
        ...
        database = Database()
        pie_data = database.execute_query("select genre, count(title) from books GROUP BY genre ORDER BY 2 DESC LIMIT 5;")
        bar_data = database.execute_query("SELECT a.title, (datediff(b.to_date, b.from_date))/(datediff(curdate(), DATE_SUB(curdate(), interval 2 year))) from books a, borrowings b WHERE a.book_id=b.book_id ORDER BY 2 DESC LIMIT 3;")
        ...
        line_data = database.execute_query("SELECT YEAR(from_date) AS year, MONTH(from_date) AS month, count(borrowed_id) as borrow_count from borrowings group by year(from_date), month(from_date) order by year(from_date), month(from_date) LIMIT 12")
        ...
        history_text = ctk.CTkLabel (self.history_frame, text=history, wraplength=300, font=ctk.CTkFont(size=15), fg_color='white')
        history_title = 'The history of Alexandria library'
        history_title_text = ctk.CTkLabel (self.history_frame, text=history_title, wraplength=280, font=ctk.CTkFont(size=18), fg_color='white')
        ...
        percentage_text = ctk.CTkLabel (self.percentage_container, text='available books', wraplength=100, fg_color='white', font=ctk.CTkFont(size=14))
        percentage_text.grid (row=0, column=0)
        percentage_counter = ctk.CTkLabel (self.percentage_container, text=f'{percentage}%', fg_color='white', font=ctk.CTkFont(size=24))
        ...
        client_counter = ctk.CTkLabel (self.client_container, text=f'{client_data[0][0]}', fg_color='white', font=ctk.CTkFont(size=24))
        ...
        books_counter = ctk.CTkLabel (self.books_container, text=f'{books_data[0][0]}', fg_color='white', font=ctk.CTkFont(size=24))
