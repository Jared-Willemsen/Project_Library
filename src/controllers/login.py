class LoginController:
    ...
    def login(self):
        ...
        if self.model.auth.login(email, password):
            ...
            self.frame.show_messagebox(title='Required fields', message='Please enter email', header=True, icon='warning')
        elif password == '':
            self.frame.show_messagebox(title='Required fields', message='Please enter password', header=True, icon='warning')
        else:
